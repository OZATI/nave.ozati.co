...
export default function Component({
  ...
  useEffect(() => {
    ...
    if ((window as any).THREE) {
      cleanUpFn = initThree((window as any).THREE);
    } else {
  ...
  const defaultLogo = (
    <div
      ...
      className="transition-transform duration-500 hover:scale-110 group cursor-pointer"
      title={brandName}
    >
      <NaveLogo variant="icon" size={26} className="text-white group-hover:rotate-180 transition-transform duration-700" />
    </div>
  );
  ...
  const Footer = (
    <div style={{ marginTop: "0.85rem", fontSize: "0.75rem", color: "#666", lineHeight: 1.5, textAlign: "center" }}>
      By proceeding, you agree to creating a {brandName} account<br />subject to our{" "}
      ...
      <div style={{ position: "relative", zIndex: 2, background: "#121212", borderRadius: 12, padding: "2rem", width: "100%", maxWidth: 400, boxShadow: "0 10px 40px rgba(0,0,0,0.8)", display: "flex", flexDirection: "column", alignItems: "center", border: "1px solid #222" }}>
        ...
        {isLogin ? (
          <div style={{ width: "100%", maxWidth: 360, display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center" }}>
            {Logo}
            <h1 style={{ fontSize: "1.35rem", fontWeight: 600, marginBottom: "0.25rem", letterSpacing: "-0.025em" }}>Sign in to {brandName}</h1>
            <p style={{ fontSize: "0.85rem", color: "#888", marginBottom: "0.85rem", lineHeight: 1.5 }}>Sign in to your {brandName} Account.</p>

            <form onSubmit={handleFormSubmit} style={{ width: "100%", display: "flex", flexDirection: "column", gap: "0.65rem" }}>
              <input
                style={input}
                type="email"
            ...
            <button type="button" style={socialBtn}>{GoogleIcon}Continue with Google</button>
            <button type="button" style={socialBtn}>{GitHubIcon}Continue with GitHub</button>
            <button type="button" style={{ ...socialBtn, marginBottom: 0 }}>{AppleIcon}Continue with Apple</button>
          ...
          <div style={{ width: "100%", maxWidth: 360, display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center" }}>
            {Logo}
            <h1 style={{ fontSize: "1.35rem", fontWeight: 600, marginBottom: "0.25rem", letterSpacing: "-0.025em" }}>Sign up for {brandName}</h1>
            ...
            <button type="button" style={socialBtn}>{GoogleIcon}Sign up with Google</button>
            <button type="button" style={socialBtn}>{GitHubIcon}Sign up with GitHub</button>
            <button type="button" style={{ ...socialBtn, marginBottom: 0 }}>{AppleIcon}Sign up with Apple</button>
